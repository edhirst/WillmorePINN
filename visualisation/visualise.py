...
def visualise_training_evolution(
    ...
    if genus_ckpt == 2:
        ...
        n_t1 = num_test_points // 2
        n_t2 = num_test_points - n_t1
        ...
        uv_t2_test = sample_torus_excluding_disk(
            ...
            disk_radius=_tmp_model.disk_radius, device=device)
        uv_test = torch.cat([uv_t1_test, uv_t2_test], dim=0)
        uv_genus2_parts = (uv_t1_test, uv_t2_test)
        precomputed_colors = make_genus2_colors(
            uv_t1_test, uv_t2_test,
...
def visualise_single_model(
    ...
    if genus == 2:
        ...
        uv_t2 = sample_torus_excluding_disk(
            ...
            disk_radius=model.disk_radius, device=device)
        uv_test = torch.cat([uv_t1, uv_t2], dim=0)
